...
(* take in a string, s, and print it with purple color *)
let print_purple (s : string) : unit =
  print_string ("\027[34m" ^ "\027[1m" ^ s ^ "\027[0m") ;;

(* take in a string, s, and print it with red color *)
let print_red (s : string) : unit =
  print_string ("\027[31m" ^ "\027[1m" ^ s ^ "\027[0m") ;;
...
let check_expect (input : 'a) (expected : 'a) : 'a check_result =
  if (input = expected) then (print_purple "Test_Passed " ; Test_Passed)
  else (print_red "Test_Failed " ;
...
let check_within (input : float) (expected : float) (within : float) : float check_result =
  if abs_float (input -. expected) <= abs_float within then (print_purple "Test_Passed " ; Test_Passed)
  else (print_red "Test_Failed " ; Test_Failed (Actual_Result (input), Expected_Result (expected))) ;;
...
let check_error (input : unit -> 'a) (expect : string) : 'a check_result =
  try
    ignore (input());
    print_red "Test_Failed " ;
    Test_Failed (Actual_Result (input()), Expected_Error (expect))
  with
    | (Failure err) when err = expect -> print_purple "Test_Passed " ; Test_Passed
    | (Failure err) -> print_red "Test_Failed " ;
